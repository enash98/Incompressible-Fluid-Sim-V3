...
def heat_step( u, v, Q, Q_bnd, Sq, k, rho ):
    uu, vv = spar.diags( [u], [0] ), spar.diags( [v], [0] )
    mat = Itens - dt * ( - uu @ Dx - vv @ Dy + k/rho * Lap_heat )
    vec = Q + dt * ( k/rho * Q_bnd + Sq )
    return spar.linalg.spsolve( mat, vec )
...
def fluid_step( u, v, Q, Q_bnd, Sx, Sy, Sq, mu, k, rho ):
    u0, v0 = fluid_momentum_step( u, v, Sx, Sy, mu, rho )
    Q1 = heat_step( u, v, Q, Q_bnd, Sq, k, rho )
    u1, v1 = Chorin( u0, v0 )
    return u1, v1, Q1
...
# -- Dirichlet Laplacian Boundary Term
...
# Q0 = ( 1 - X/Lx ) * ( 0.8 + 0.2 * ( 1 + np.cos( np.pi * Y / Ly ) )/2 )
...
# Q_bnd = np.zeros_like(X[1:-1, 1:-1])
# Q_bnd[0], Q_bnd[-1] = 1/dx**2 * Q0[0, 1:-1], 1/dx**2 * Q0[-1, 1:-1]
# Q_bnd = Q_bnd.flatten()



# Non-vanishing 'boundary-term' for the first derivative in the y-direction
W = boundary1_dir( Q0 )[1]


# -- Source Terms

# Sy0 = 4 * ( funx * funy )[1:-1, 1:-1].flatten()
Sx, Sy = np.zeros_like(u), - beta * ( Dy @ Q + W )
...
with writer.saving( fig, 'New Heat Plot.gif', 10*tmax+1 ):
    for t in tvals:
        ...
        Sy = - beta * ( Dy @ Q + W )
        u,v,Q = fluid_step( u, v, Q, Q_bnd, Sx, Sy, Sq, mu, k, rho )
